...
## 8. 오브 보상 루프 — 비주얼 신호 시스템

> **Purpose**: 적 처치 → 오브 지급 → 누적 → 업그레이드 선택 → 효과 즉시 적용, 5단계가 플레이어 인지 안에서 끊김 없이 연결되도록 비주얼 신호를 완성한다. 연출 추가가 목적이 아니라 루프 각 단계를 플레이어가 읽을 수 있게 하는 것이 목적이다.

---

### 8-0. 게임 로직 확정값 (프로토타이핑 기준 — 수치 한 세트 교체 시 전면 재작업 없이 반영되는 구조로 구현)

| 항목 | 확정값 | VFX·UI에 미치는 영향 |
|------|--------|----------------------|
| 오브 지급 단위 | 처치당 1개 고정 (적 종류·레벨 무관) | 파티클 수·크기 기준 상수 |
| 누적 임계치 | 10개 = 업그레이드 1회 트리거 | 카운터 슬롯 수·색 구간 기준 |
| 업그레이드 선택지 수 | 3개 (기존 4-3 카드 레이아웃 재사용) | 카드 열 수·아이콘 크기 기준 |
| 대표 효과 수치 | ATK +20% / SPD +15% / HP +25% | 연출 강도(글로우 세기) 기준 |
| **처치→UI 직렬 총합 상한** | **≤900ms** [2R 확정] | QA 항목 7 판정 기준; 단계별 최악 합산 100+380+300+200=980ms → 여유 포함 900ms. 병렬 실행 시 기대값 ≤500ms |

**트리거 방식**: 오브 임계치 도달 → 기존 `augment_select` 페이즈 진입 (게임 일시 정지, 풀스크린 오버레이). 웨이브 클리어 업그레이드와 동일 UI·이벤트 재사용. 두 트리거는 중복 시 오브 우선 처리 후 웨이브 클리어 처리.

---

### 8-1. 서버 이벤트 계약 (백엔드 ↔ 프론트엔드 / VFX 합의 인터페이스)

| 이벤트 type | 발생 시점 | 필수 필드 | 주의 |
|-------------|-----------|-----------|------|
| `orb_grant` | 적 처치 판정 즉시 (같은 tick) | `pid`, `x`, `y`, `orbCount`, `threshold` | threshold 고정값 10; 클라이언트 VFX 발사 기준점 |
| `orb_threshold` | orbCount가 threshold에 도달하는 tick | `pid` | 직후 같은 tick에 `augment_offer` 발생 |
| `augment_offer` | orb_threshold 또는 wave_clear 직후 | 기존 구조 유지 | 오브 트리거 시 `source: 'orb'` 필드 추가(UI 구분 가능하도록) |
| `orb_revoke` | 서버 취소 판정 시 (치팅 감지·레이스 컨디션 수정 등) [2R 추가] | `pid`, `orbCount`(서버 정정값) | 클라이언트 즉시 롤백; 200ms 이내 가시 신호 필수 |

**서버 상태(snapshot) 추가 필드**:
- `players[n].orbCount` (0~9, 임계치 초과분은 이월)
- `players[n].orbThreshold` (상수 10, 클라이언트 바 구간 계산용)

**오브 카운트 초기화 규칙**: 임계치 도달 즉시 `orbCount = 0`. 업그레이드 선택 중 추가 처치는 다음 사이클로 이월.

---

### 8-2. Stage 1 → 2: 오브 지급 VFX (VFX 전문가 담당)

**타이밍 기준**: `orb_grant` 이벤트 수신 후 **100ms 이내** 파티클 생성.

| 파라미터 | 값 |
|----------|----|
| 생성 위치 | 처치된 적의 (x, y) |
| 형태 | 구체, r=7px |
| 색상 | 코어 #FFFFFF, 글로우 #FFD700, shadowBlur=12 #FFAA00 |
| 비행 목표 | HUD 오브 카운터 위치 (우하단, 8-3 참조) |
| 비행 궤적 | arc 포물선 (상방 20~40px 최고점 경유) |
| 비행 시간 | 380ms, easeInBack |
| 도착 연출 | 목표 슬롯에 닿는 순간 r=7→0px (흡수), 금색 미니 파티클 3개 방사 |
| 동시 다중 처치 | **첫 orb_grant 수신 t=0 기준**: 첫 오브 VFX spawn ≤100ms, N번째 VFX spawn ≤100+(N-1)×50ms [2R 확정 — 독립 100ms 아님, 첫 오브가 묶음 기준점] |
| 파티클 예산 | PART_CAP과 독립: 오브 파티클은 별도 pool(최대 8개) |

**Effects API 확장**: `Effects.spawn('orb_grant', { x, y, targetX, targetY, onArrive: cb })` — `onArrive` 콜백으로 카운터 슬롯 ON 신호를 프론트엔드에 전달.

> **[2R 미결 — 3자 합의 대기 중]** VFX spawn 타입 토큰 제안: `Effects.spawn` 첫 인수 `'orb_grant'` → `'orb_arc'` 변경(서버 소켓 이벤트명과 혼동 방지), 도착 emit `'orb_arrive'` 신규 추가. VFX 전문가·프론트엔드 확인 후 이 행 확정. **합의 전 QA 자동화 핸들러 작성 불가.**

---

### 8-2.5. 롤백 처리 (`orb_revoke` 수신 시) [2R 추가]

| 상황 | 클라이언트 동작 | 수치 기준 |
|------|----------------|-----------|
| 파티클 비행 중 | 파티클 즉시 소멸 (fade-out 80ms) | 수신 후 ≤1프레임(16ms) 이내 cancelling 상태 전환 |
| 슬롯 ON 상태 | 슬롯 OFF 전환 (fill #FFD700→#111133, 200ms) | 수신 후 **200ms 이내** 전환 완료 |
| UPGRADE! 텍스트 표시 중 | 텍스트 즉시 fade-out (100ms) + 전체 슬롯 roll-back | 수신 후 **200ms 이내** 가시 신호 |
| augment_select 이미 표시 | 강제 닫힘(fade 200ms) + 오브 카운터 `orbCount` 서버 정정값으로 강제 동기 | 수신 후 **200ms 이내** 완료 |

**공통 기준**: `orb_revoke` 수신 후 **200ms 이내** 가시 롤백 신호 발생. VFX 파티클 취소와 UI 슬롯 롤백은 별도 QA 검증 시나리오로 분리.

---

### 8-3. Stage 2 → 3: 오브 카운터 HUD (게임 비주얼 디자이너·프론트엔드 담당)

**위치**: 우하단, 킬 카운터(`⚔`) 위 16px 여백.
- 기준점: x=1264 right-align, y=675 (킬 카운터 y=705 기준 위)

**슬롯 10개 가로 배열**:

| 상태 | fill | stroke | shadowBlur |
|------|------|--------|------------|
| 빈 슬롯 | #111133 | #333355 lineWidth=1 | 0 |
| 채워진 슬롯 | #FFD700 | #FFAA00 lineWidth=1 | 6 #FFAA00 |

- 슬롯 크기: r=5px (원형)
- 슬롯 간격: 6px (총 너비 ≈116px)
- 카운트 텍스트: `[n]/10`, font 11px #AAAACC, 배열 우측 10px

**일반 증가 애니메이션** (orbCount n→n+1):
- 해당 슬롯: scale 1.0 → 1.7 → 1.0, 180ms, easeOutBack
- 미니 파티클 2개: 슬롯 상방 40px 부유 → fade, 300ms, col=#FFD700

**임계치 도달 특별 연출 (일반 증가와 시각적으로 명확히 구별)**:
1. `orb_threshold` 수신 즉시: 전체 슬롯 row 흰색 오버레이 alpha=0.9 → 0, **100ms**
2. +0ms: 금색 링 r=16px → 80px 팽창, strokeWidth=4→1, **300ms** easeOutCirc, col=#FFD700
3. +50ms: "UPGRADE!" 텍스트, font bold 20px #FFD700, shadowBlur=10, 슬롯 row 위 y-28px
   - scale 0 → 1.2 → 1.0 (200ms easeOutBack) → **0.8s** 유지 → fade 200ms
4. +100ms: 전체 슬롯 scale 1.0 → 1.15 → 1.0, 400ms inOutSine
5. +300ms: augment_select UI 등장 (기존 4-3)

---

### 8-4. Stage 4: 업그레이드 선택 UI

기존 **4-3 증강 선택 UI** 그대로 사용. 추가 규칙:

- `source: 'orb'`인 경우 타이틀 서브텍스트 변경: `"오브 10개 누적 — 업그레이드를 선택하세요"` (font 14px #AAAACC)
- 오브 카운터 HUD: 선택 오버레이 활성 중 **숨김** (최상위 레이어 오버레이가 가리므로 z-order 자연 처리)
- 카드 3장 동시 표시 (순차 등장 금지 — QA 기준)
- 각 카드: 텍스트(이름+등급+설명) + 아이콘(색+형태) 양방향으로 구별 (색맹 대응)

**카드별 아이콘 색 규약** (대표 효과 기준):
| 효과 유형 | 아이콘 배경색 | 테두리 | 이유 |
|-----------|--------------|--------|------|
| 공격력 계열 | #AA2200 | #FF4444 | 붉은 계열 = 공격 |
| 이동속도 계열 | #1A4A8A | #4A90E2 | 파랑 계열 = 기동 |
| HP·회복 계열 | #1A5A2A | #44DD44 | 초록 계열 = 생존 |

---

### 8-5. Stage 4 → 5: 효과 즉시 적용 비주얼 (선택 후 200ms 이내)

**발동 흐름**: 카드 선택 → `augment_aura` (기존 3-9) 200ms 이내 발동 → 유형별 추가 연출 덧씌움.

| 업그레이드 유형 | 캐릭터 추가 연출 | HUD 추가 연출 | 지속 |
|----------------|----------------|--------------|------|
| 공격력 +20% | 무기 방향 금빛 선폭 flash (lineWidth×3→1, 300ms) | - | 300ms |
| 이동속도 +15% | 발 아래 파란 발자국 파티클 4개 burst, 상방 60px float | 이동 중 blue trail 1.5s | 600ms |
| HP +25% | 초록 힐 링 r=20→90px 팽창 (outCirc, 400ms) + 초록 파티클 8개 | HP바 초록 글로우 pulse 300ms | 400ms |

**공통 규칙**: 어떤 업그레이드든 선택 후 **화면 어딘가에 200ms 이내 가시 변화**가 발생해야 함. `augment_aura` 링이 이 기준을 충족하지 못하는 경우(지연) VFX 파이프라인 우선 점검.

---

### 8-6. QA 체크리스트 (전 항목 1회 통과 = 완료 조건)

| # | 항목 | 검증 방법 | 통과 기준 |
|---|------|-----------|-----------|
| 1 | 오브 지급 VFX 타이밍 | 처치 이벤트 타임스탬프 vs VFX 첫 프레임 측정 | 100ms 이내 |
| 2 | 오브 카운터 즉시 갱신 | 오브 파티클 도달 순간 슬롯 ON 확인 | 비행 완료 즉시 (프레임 지연 없음) |
| 3 | 임계치 연출 구별 | 1→2번째 오브 증가 vs 9→10번째 오브 도달 화면 비교 | 시각적으로 명확히 다름 (규모·색·텍스트) |
| 4 | 선택지 3장 동시 표시 | 카드 등장 시 3장 동시 렌더 확인 | 순차 등장 없음 |
| 5 | 텍스트+아이콘 양방향 구별 | 색 없이 형태만으로 / 형태 없이 색만으로 각 카드 식별 | 둘 다 통과 |
| 6 | 효과 적용 가시 | 카드 선택 타임스탬프 vs 캐릭터·HUD 변화 첫 프레임 | 200ms 이내 |
| 7 | 전체 루프 연속성 | 처치→오브→누적→선택→적용 한 바퀴 실제 플레이; kill 이벤트 타임스탬프 → augment_select 카드 표시까지 측정 | 5단계 끊김 없이 연결 + **총합 ≤900ms** (직렬 최악 기준) |
| 8 | 경계: 오브 0 상태 시작 | 게임 시작 직후 오브 카운터 상태 확인 | 슬롯 10개 모두 빈 상태 |
| 9 | 경계: 다중 처치 동시 | 범위기 등으로 N(≥2)적 동시 처치 시 오브 발사 타임스탬프 측정 | 첫 오브 VFX spawn ≤100ms; N번째 ≤100+(N-1)×50ms **(첫 오브 기준, 독립 100ms 아님)** [2R 확정] |
| 10 | 중복 트리거 방지 | 업그레이드 선택 중 10번째 처치 시 | 선택 완료 후 다음 사이클 정상 시작 |
| 11 | 롤백 가시화 [2R 추가] | `orb_revoke` 이벤트 주입 후 클라이언트 화면 측정 (a) 파티클 비행 중 주입 (b) 슬롯 ON 후 주입 | 수신 후 **200ms 이내** 가시 신호 — (a) 파티클 fade-out (b) 슬롯 OFF 전환. 두 시나리오 별도 검증 |
| 12 | arc 이벤트 타입명 자동화 [2R 추가 — 착수 불가] | 3자 토큰 합의(VFX·FE·기획) 완료 확인 후 자동화 핸들러 작성 | **합의 완료 전 이 항목 착수 불가** — VFX 전문가 확인 대기 중 |
