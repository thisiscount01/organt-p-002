...
(function () {
  ...
  // ── 모듈 상태(프레임간 지속) ──────────────────────────────────────────
  const _edead  = new Map(); // eid→{at}  사망 시작 시각
  const _ealive = new Map(); // eid→bool  이전 프레임 생존 여부

  function _trackEnemyDeath(eid, dead, now) {
    const was = _ealive.get(eid);
    _ealive.set(eid, !dead);
    if (dead && was !== false && !_edead.has(eid)) _edead.set(eid, { at: now });
    if (!dead && _edead.has(eid)) _edead.delete(eid);
  }

  // 사망 파티클 디졸브 (원점 기준, ctx는 enemy 위치로 translate된 상태)
  function _enemyDeathDissolve(ctx, m, r, elapsed, DUR) {
    // 흰 플래시 (0~120ms)
    if (elapsed < 120) {
      const f = 1 - elapsed / 120;
      ctx.save();
      ctx.globalAlpha = f * 0.88;
      ctx.fillStyle = '#FFFFFF';
      ctx.shadowColor = '#FFFFFF'; ctx.shadowBlur = 16;
      ctx.beginPath(); ctx.arc(0, 0, r * (1 + f * 0.45), 0, TAU); ctx.fill();
      ctx.shadowBlur = 0;
      ctx.restore();
    }
    // 파편 산란 (45ms~)
    if (elapsed > 45) {
      ...
      ctx.save();
      for (let i = 0; i < 7; i++) {
        const ang = (i / 7) * TAU + 0.55;
        const d = ease.outQuad(tp) * r * 2.5;
        const sr = r * 0.29 * (1 - tp * 0.88);
        if (sr < 0.3) continue;
        ctx.globalAlpha = (1 - tp) * 0.80;
        ctx.fillStyle = i % 2 ? m.c : (m.hi || m.c);
        ctx.beginPath();
        ctx.arc(Math.cos(ang) * d, Math.sin(ang) * d, sr, 0, TAU);
        ctx.fill();
      }
      ctx.restore();
    }
  ...
  function drawEnemy(ctx, e, now, opts) {
    opts = opts || {};
    const eid = e.id != null ? e.id : (e.x + '|' + e.y);
    ...
    // 사망 추적
    _trackEnemyDeath(eid, !!e.dead, now);
    const deathInfo = _edead.get(eid);
    const deathElapsed = (e.dead && deathInfo) ? (now - deathInfo.at) : 9999;
    const deathAnimating = e.dead && deathElapsed < 600;
    const deathT = deathAnimating ? clamp(deathElapsed / 600, 0, 1) : 0;

    // 이동 스쿼시-스트레치 (e.vx/vy 활용)
    const espeed = Math.sqrt((e.vx || 0) * (e.vx || 0) + (e.vy || 0) * (e.vy || 0));
    const moveT = clamp(espeed / 200, 0, 1);
    const moveDir = espeed > 20 ? Math.atan2(e.vy || 0, e.vx || 0) : 0;

    // 유휴 맥동 (공격·사망 중 아닐 때)
    const isIdle = !mo.type && !e.dead && espeed < 30;
    const idlePhase = ((eid || 0) * 6571) % (Math.PI * 2);
    const idlePeriod = 850 + Math.sin(idlePhase) * 120; // 730~970ms 개체별 편차
    const idleSc = isIdle ? (1 + Math.sin(now * Math.PI * 2 / idlePeriod + idlePhase) * 0.035) : 1;
    ...
    // 사망: 600ms 애니메이션 후 렌더 생략 (즉사 느낌 제거)
    if (e.dead) {
      if (!deathAnimating) { ctx.restore(); return; }
      ctx.globalAlpha = clamp(1 - ease.outQuad(deathT), 0, 1);
    }
    ...
    // 스케일 합성: 모션 스쿼시 + 이동 스트레치 + 유휴 맥동 + 사망 축소
    let scX = mo.sx, scY = mo.sy;
    if (deathAnimating) {
      const ds = 1 - ease.outQuad(deathT) * 0.68;
      scX *= ds; scY *= ds;
    }
    if (moveT > 0.05 && !e.dead) {
      ctx.rotate(moveDir);
      scX *= (1 + moveT * 0.09);
      scY *= (1 - moveT * 0.07);
      ctx.rotate(-moveDir);
    }
    if (isIdle) { scX *= idleSc; scY *= idleSc; }
    ...
    // 사망 파티클 디졸브 (외부 컨텍스트 — globalAlpha 곱 없이 독립)
    if (deathAnimating) {
      ctx.save();
      ctx.translate(e.x + ox, e.y + oy);
      _enemyDeathDissolve(ctx, m, r, deathElapsed, 600);
      ctx.restore();
    }
